...
class Phase13RemainingGateTechnicalAuditTests(unittest.TestCase):
  ...
  def test_audit_maps_all_open_markers_and_keeps_promotion_blocked(self):
    ...
    self.assertEqual(self.audit["package_version"], "0.13.108")
    ...
    reference_check = next(check for check in self.audit["technical_checks"] if check["id"] == "checkpoint-reference-transfer")
    self.assertIn("gui/app.mjs", reference_check["sources"])
    artifact_check = next(check for check in self.audit["technical_checks"] if check["id"] == "checkpoint-artifact-download")
    ...
    campaign_check = next(check for check in self.audit["technical_checks"] if check["id"] == "firefox-multi-campaign-launch-smoke")
