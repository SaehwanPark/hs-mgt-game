...
LEDGER = ROOT / "docs" / "evaluation" / "phase12-stabilization-provenance-audit.json"


class Phase12StabilizationProvenanceAuditTests(unittest.TestCase):
  @classmethod
  def setUpClass(cls):
  ...
  def test_provenance_sources_and_audit_checks_match(self):
    ledger = self.ledger
    self.assertEqual(ledger["schema_version"], "stabilization-provenance-audit-v1")
    ...
    self.assertEqual(len(ledger["audit_checks"]), 6)
  ...
  def test_campaign_provenance_and_release_boundaries_remain_bounded(self):
    ...
    for marker in ("technical provenance audit", "legal clearance", "No new map", "Unverified portrait previews", "public-release", "educational"):
      self.assertIn(marker, limits)
    self.assertEqual(self.ledger["test_source"], "tests/test_phase12_stabilization_provenance_audit.py")
