...
- Preserve host-reported history/hash evidence in committed GUI playtest
  captures and add a regression guard for the visible-envelope recorder path.
...
- Added a source-bound debrief visual review packet for the three corrected
  terminal campaign cases, with written-equivalent, accessibility/fallback,
  causality, replay, provenance, and structured human-review questions.
- Kept human visual/accessibility, educational/classroom, audio-listening,
  provenance/legal, and public-release decisions explicitly pending.
...
- Recorded the Phase 12 reusable-asset matrix for stabilization and regional
  affiliation against the existing visual, generated-audio, and facility
  fallback catalogs.
- Closed only the reusable-assets checklist items; direct campaign audio
  mapping, campaign-specific art/stage treatment, quality, human review, and
  educational gates remain open, with no runtime or asset behavior changed.
...
- Added a distinct fictional research-and-education-building source/release
  facility component with base, identity, capacity, project, pressure,
  selection, and uncertainty layer vocabulary.
- Extended the shared facility proof and catalog with research-and-education
  building selection, generic fallback, registry hashes, accessible labels,
  and non-color patterns.
- Completed the research-and-education-building per-component Phase 3.1
  checklist without changing live GUI or host behavior.
- Bumped package/public metadata to `0.12.52`.
...
- Added a distinct fictional parking-structure source/release facility
  component with base, identity, capacity, project, pressure, selection, and
  uncertainty layer vocabulary.
- Extended the shared facility proof and catalog with parking-structure
  selection, generic fallback, registry hashes, accessible labels, and
  non-color patterns.
- Completed the parking-structure per-component Phase 3.1 checklist without
  changing live GUI or host behavior.
- Bumped package/public metadata to `0.12.50`.

## [0.12.49] - 2026-07-20

- Added a distinct fictional administrative-headquarters source/release
  facility component with base, identity, capacity, project, pressure,
  selection, and uncertainty layer vocabulary.
- Extended the shared facility proof and catalog with administrative-headquarters
  selection, generic fallback, registry hashes, accessible labels, and
  non-color patterns.
- Completed the administrative-headquarters per-component Phase 3.1 checklist
  without changing live GUI or host behavior.
- Bumped package/public metadata to `0.12.49`.

## [0.12.48] - 2026-07-20

- Added a distinct fictional rural-clinic source/release facility component
  with base, identity, capacity, project, pressure, selection, and uncertainty
  layer vocabulary.
- Extended the shared facility proof and catalog with rural-clinic selection,
  generic fallback, registry hashes, accessible labels, and non-color patterns.
- Completed the rural-clinic per-component Phase 3.1 checklist without
  changing live GUI or host behavior.
- Bumped package/public metadata to `0.12.48`.

## [0.12.47] - 2026-07-20

- Added a distinct fictional specialty-center source/release facility
  component with base, identity, capacity, project, pressure, selection, and
  uncertainty layer vocabulary.
- Extended the shared facility proof and catalog with specialty-center
  selection, generic fallback, registry hashes, accessible labels, and
  non-color patterns.
- Completed the specialty-center per-component Phase 3.1 checklist without
  changing live GUI or host behavior.
- Bumped package/public metadata to `0.12.47`.

## [0.12.46] - 2026-07-20

- Added a distinct fictional ambulatory-center source/release facility
  component with base, identity, capacity, project, pressure, selection, and
  uncertainty layer vocabulary.
- Extended the shared facility proof and catalog with ambulatory-center
  selection, generic fallback, registry hashes, accessible labels, and
  non-color patterns.
- Completed the ambulatory-center per-component Phase 3.1 checklist without
  changing live GUI or host behavior.
- Bumped package/public metadata to `0.12.46`.

## [0.12.45] - 2026-07-20

- Added a distinct fictional emergency-department source/release facility
  component with base, identity, capacity, project, pressure, selection, and
  uncertainty layer vocabulary.
- Extended the shared facility proof and catalog with emergency-department
  selection, generic fallback, registry hashes, accessible labels, and
  non-color patterns.
- Completed the emergency-department per-component Phase 3.1 checklist without
  changing live GUI or host behavior.
- Bumped package/public metadata to `0.12.45`.

## [0.12.44] - 2026-07-20

- Added a distinct fictional patient-tower source/release facility component
  with base, identity, capacity, project, pressure, selection, and uncertainty
  layer vocabulary.
- Extended the shared facility proof and catalog with patient-tower selection,
  generic fallback, registry hashes, accessible labels, and non-color patterns.
- Completed the patient-tower per-component Phase 3.1 checklist without
  changing live GUI or host behavior.
- Bumped package/public metadata to `0.12.44`.
...
### Added
- Added a deterministic read-only debrief-use audit across six existing Phase
  7 evidence artifacts.
- Added focused coverage for event-specific visibility, response, follow-through,
  outcome, explanation, and project-recovery hash continuity.

### Changed
- Kept runtime, MCP behavior, scenarios, replay formats, state hashes, scoring,
  difficulty, balance, and debrief wording unchanged.
- Kept runtime promotion deferred because the reviewed source shapes identify no
  unexplained traceability gap.
- Bumped the package version to `0.10.57`.
...
### Changed
- Routed the next bounded follow-up toward instructor-facing comparison or
  broader strategy-space synthesis while keeping runtime mechanics, command
  costs, pledge cooldowns, difficulty changes, and balance changes deferred.
- Bumped the package version to `0.10.26`.
...
### Changed
- Routed the next bounded evidence gate toward access-heavy player understanding
  of public pledges versus durable operational follow-through, while keeping
  runtime tuning deferred.
- Bumped the package version to `0.10.21`.
...
- Added live retry signal reporting to `scripts/diagnose_runs.py` for
  live-capture artifacts that include optional `live_validation_retries`
  metadata.

### Changed
- Regenerated `_workspace/experiments/v0.10.15-live-llm-difficulty-gate/diagnostics.md`
  with final-validation versus live-retry separation.
- Bumped the package version to `0.10.17`.
