...
impl McpGameServer {
  ...
  #[tool(
    name = "save_session",
    description = "Save an explicit host checkpoint for a live session without serializing state in the client; the loopback GUI may persist competitive or stabilization checkpoints in its host-owned file."
  )]
  ...
  #[tool(
    name = "load_session",
    description = "Restore an in-memory host checkpoint or a matching durable loopback competitive or stabilization checkpoint and return aligned save metadata without client-side state restoration."
  )]
