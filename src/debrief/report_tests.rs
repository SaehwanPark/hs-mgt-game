...
fn test_competitive_decision_quality_warnings() {
  ...
  fn test_competitive_debrief_rationale_visibility() {
    ...
    //    revealed for instructor.
    {
      ...
      assert!(!debrief_str.contains("Rival private rationale test"));
      assert!(debrief_str.contains("[Private Action] (unobserved by you)"));
    ...
    // 2. Private action only, but monitored -> Rationale should be observed via monitor
    {
      ...
      assert!(debrief_str.contains("Rival monitored rationale test (observed via monitor)"));
    ...
    // 3. Public action present, not monitored -> Rationale should be observed via public disclosure
    {
      ...
      assert!(debrief_str.contains("Rival public rationale test (observed via public disclosure)"));
