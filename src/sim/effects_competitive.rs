...
pub fn apply_month_start_tick(
  ...
  if world.scenario_id == "exemplary-competitive-v1" {
    ...
    if month_index >= 18 && world.event_metadata.get("ehr_project_fully_funded") != Some(&"true".to_string()) {
      if let Some(riverside) = world.systems.iter_mut().find(|sys| sys.system_id == 0) {
    ...
    if month_index >= 12 && world.event_metadata.get("blue_shield_negotiated") != Some(&"true".to_string()) {
      if world.event_metadata.get("blue_shield_out_of_network") != Some(&"true".to_string()) {
        world.event_metadata.insert("blue_shield_out_of_network".to_string(), "true".to_string());
        if let Some(riverside) = world.systems.iter_mut().find(|sys| sys.system_id == 0) {
          riverside.market_share_index = (riverside.market_share_index as f32 * 0.60).round() as i32;
          events.push(Event {
            actor: "payer",
    ...
    if month_index == 10 {
      ...
      {
        world.event_metadata.insert("rna_strike_active".to_string(), "true".to_string());
        world.event_metadata.insert("rna_strike_months_left".to_string(), "2".to_string());
        events.push(Event {
          actor: "workforce",
      ...
      if has_clinic_project {
        ...
        if let Some(riverside) = world.systems.iter_mut().find(|sys| sys.system_id == 0) {
          if riverside.resources.political_capital >= 3 {
            ...
            for effect in &mut world.effect_queue {
              if effect.system_id == 0 && matches!(effect.kind, PendingEffectKind::OutpatientCapacity { .. }) {
                effect.resolve_month += 3;
              }
            }
            events.push(Event {
              actor: "regulator",
              description: "Clinic project suspended for 3 months due to CON legal challenge.".to_string(),
            });
          }
        }
      }
      ...
      if let Some(months_left_str) = world.event_metadata.get("rna_strike_months_left").cloned() {
        if let Ok(months_left) = months_left_str.parse::<u32>() {
          if months_left > 0 {
            ...
            world.event_metadata.insert("rna_strike_months_left".to_string(), new_left.to_string());
            if new_left == 0 {
              world.event_metadata.insert("rna_strike_active".to_string(), "false".to_string());
              events.push(Event {
                actor: "workforce",
                description: "RNA strike has ended. Riverside operations return to normal.".to_string(),
              });
            }
          }
        }
    ...
    if month_index == 18 {
      if world.event_metadata.get("rna_strike_active") == Some(&"true".to_string()) || world.event_metadata.get("rna_strike_months_left").is_some() {
        if let Some(riverside) = world.systems.iter_mut().find(|sys| sys.system_id == 0) {
