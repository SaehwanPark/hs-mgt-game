...
[`mcp-playtesting-guide.md`](../guides/mcp-playtesting-guide.md):
...
`docs/history/playtests/v0.1/playtest-findings-v0.1.48.md`. Do not overwrite
prior findings.
...
## Future Human Playtesting Safeguards

Human playtesting is not part of the active validation path. The guidance below
is retained from the superseded v0.1.39 external-playtest protocol so a future,
separately approved plan does not lose its reusable safeguards. Do not reuse the
old campaign scripts as a current research or recruitment protocol.

Before an informal product session, explain that the game is a fictional
educational prototype, avoid coaching toward a preferred strategy, keep
timestamps for major confusion points, and separate observed behavior from
interpretation. Do not use one session to tune numeric balance or infer
real-world policy effects.

If results will be used for publishable research, course assessment, employment
evaluation, or identifiable participant analysis, pause and follow the required
institutional review, consent, privacy, and data-retention process. Avoid
recording names, institutional affiliations, or other identifying details
without that approved process.

Reusable post-session prompts include:
...
Future session notes should record the session configuration, participant
background in non-identifying terms, completion status, observed confusion,
notable decisions, invalid-input friction, rubric scores, evidence limits, and
deferred requests. Synthesis should report repeated patterns rather than
