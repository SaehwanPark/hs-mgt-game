...
The v0.13.41 Phase 12.3 slice records current instructor-only authority
...
instructor or true-state browser route. Its source-linked textual labels keep
observed, true-state, and instructor-only language distinct without expanding
authority. Its decision-time evidence keeps core observations paired with
commands while documenting the narrower host/browser summary boundary. It does
...
Stabilization uses the versioned `replay-artifact-0.1.15` writer/verifier,
regional affiliation uses its versioned writer, and competitive CLI export
remains serialized `CompetitiveHistory` JSON. Empty input skips writing. These
artifacts are not browser authority, commands, mid-run saves, transitions,
hashes, or new simulation state.
