#!/usr/bin/env python3
"""Validate access-pledge guidance with bounded free-form Hard policies."""
...
def wrap_guidance_aware(policy_fn):
  def policy(obs, legal, turn, context):
    command = policy_fn(obs, legal, turn, context)
    access = parse_access(obs) or 70
    suppress_repeated = recent_access_pledge(context, turn, 2)
    suppress_at_high_access = access >= 85
    if command_has_access_pledge(command) and (suppress_repeated or suppress_at_high_access):
...
VARIANTS = {
  "baseline": {
    "description": "v0.10.1/v0.10.2 free-form policy behavior, unchanged.",
    "wrap": lambda policy_fn: policy_fn,
  },
  "guidance_aware": {
    "description": (
      "Suppress repeated access pledges after recent pledges or high reported "
      "access, then redirect to durable capacity, staffing, monitoring, payer, "
      "or hold fallback actions."
    ),
    "wrap": wrap_guidance_aware,
  },
}
...
def main():
  ...
  seeds = [42, 43, 44]
  results = {
    "artifact_type": "free_form_hard_post_guidance_validation",
    "code_version": "0.10.4",
    "campaign": "competitive-regional-v1",
    "seeds": seeds,
    "difficulty": "hard",
