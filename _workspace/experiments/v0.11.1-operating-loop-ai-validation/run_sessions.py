#!/usr/bin/env python3
"""Capture the v0.11.1 operating-loop AI policy matrix."""
...
def affordable(command, resources):
  ...
  # Leave room for active project draws, which are visible in the observation
  # but are not included in the compact legal-resource line.
  return (
...
def capture(output_path):
  ...
  if version != "0.11.1":
    raise RuntimeError(f"expected package version 0.11.1, found {version}")
  ...
  runs = []
  for profile, _ in PROFILES.items():
    for seed in SEEDS:
      for difficulty in DIFFICULTIES:
        ...
        runs.append({
          ...
          "difficulty": difficulty,
          "campaign": CAMPAIGN,
          "completion_status": "complete" if len(history) == 24 else "incomplete",
