...
The v0.13.109 technical evidence slice extends the loopback Firefox/Marionette
probe through the visible competitive Hold path. It does not promote Firefox,
WebKit, device, human, educational, provenance, or public-release gates.

## Evidence

- Plan: `_workspace/187_implementation_plan_phase13-firefox-competitive-full-campaign-smoke-v0.13.109.md`.
- Request summary: `_workspace/00_input/request-summary-v0.13.109.md`.
- Packet: `docs/evaluation/phase13.1-firefox-runtime-smoke-packet.json`.
- The packet records 24 host-validated and committed months, 24 autosave
  statuses, 24 history/replay rows with state hashes, and a host terminal
  debrief.
- The consolidated audit is current at v0.13.109 with eight promotion-blocking
  gates still mapped to authorized human or runtime evidence.

## Verification

- Rebuilt GUI host and live Firefox 147.0.2 probe passed.
- Focused packet, audit, browser/device, technical coverage, and release tests
  passed.
- Full Rust, Python, lint, formatting, device-proxy, audit, and diff checks are
  required before PR handoff.
...
# Final Handoff — player-first README and installation v0.14.13

## Status

Implemented locally on `codex/feat/player-first-readme-v0.14.13`. The root
README is now player-first and recommends the GUI stabilization tutorial with
seed `42`; CLI play, installation, troubleshooting, campaign distinctions, and
contributor links are explicit. No runtime behavior or campaign mechanics
changed.

## Changed-file groups

- Player entry point and guides: `README.md`,
  `docs/guides/installation.md`, `docs/guides/how-to-play.md`, and
  `docs/guides/gui-how-to-play.md`.
- Contributor and contract checks: `docs/README.md`,
  `scripts/check_documentation_currentness.py`, and
  `tests/test_player_documentation.py`.
- Maintained screenshots: five PNGs and `docs/images/readme/README.md`, all
  source-bound and checksum-recorded.
- Release projections: `Cargo.toml`, `Cargo.lock`, `README.md`,
  `CHANGELOG.md`, and `tests/test_release_metadata.py` at `0.14.13`.
- Generated release projections: `assets/ASSET_CREDITS.md`,
  `assets/THIRD_PARTY_NOTICES.md`, and `gui/asset-credits.mjs` regenerated by
  the repository's asset-credit generator.
- Required request, presentation-contract, domain-QA, and presentation-QA
  sections were appended to the existing `_workspace` records.

## Evidence limits and authorization gates

The local handoff records technical documentation/capture evidence only. It
does not claim human usability, learning, lived accessibility, legal or policy
validity, cross-platform certification, packaging, hosted deployment, or public
release readiness. Commit, push, PR creation, and external review remain
separate authorization gates.

## Validation run

- Documentation currentness, documentation links, release metadata, `cargo
  fmt --check`, `cargo clippy --all-targets -- -D warnings`, and `git diff
  --check`: pass.
- Focused player/GUI/release tests: 12 pass; generated asset-credit and
  first-session boundary regressions: 30 pass.
- Full `python3 -m unittest discover -s tests`: 956 pass.
- `cargo test`: 388 library tests plus integration/doc tests pass.
- Five PNGs were inspected at full resolution; all are lossless, local, and
  match the manifest SHA-256 values.

## Deviation record

No behavior or scope deviation was made. The canonical generated asset-credit
Markdown/runtime projections were regenerated to `0.14.13` because the full
repository checks require generated release metadata to match `Cargo.toml`;
this changes metadata only and does not change the runtime presentation.
